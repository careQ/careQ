...
function connect() {
    ...
    stompClient.connect({}, function (frame) {
        ...
        stompClient.subscribe('/topic/chatrooms/'+window.location.pathname.split('/')[3], function (message) {
            if(JSON.parse(message.body).userType == 'MEMBER'){
                addMemberMessage(JSON.parse(message.body).content);
            }
        });
    });
}
...
function send() {
    ...
    if (content != "") {
        let msg = {
            ...
            'sender': document.getElementById("adminName").value,
            'content': content
        }
...
function addAdminMessage(content) {
    ...
    messageDiv.innerHTML = `
                  <div class="bg-[#F6F6F6] p-2 rounded-lg max-w-[80%] relative" style="text-align: right;"> <!--오른쪽 정렬-->
